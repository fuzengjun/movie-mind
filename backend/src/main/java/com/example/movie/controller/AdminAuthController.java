...
public class AdminAuthController {
    ...
    public Result<Map<String, Object>> login(@Valid @RequestBody LoginRequest request) {
        ...
        if (!"ADMIN".equalsIgnoreCase(String.valueOf(user.get("role"))))
            throw new ResponseStatusException(UNAUTHORIZED, "当前账号没有后台权限");
        if (!stored.startsWith("$2"))
            jdbcTemplate.update("UPDATE user SET password=?,update_time=NOW() WHERE id=?", passwordEncoder.encode(request.getPassword()), user.get("id"));
        user.remove("password");
        return Result.success(Map.of("token", jwtUtil.createToken(request.getUsername()), "user", user));
    }
}
