...
public class AdminManagementServiceImpl implements AdminManagementService {
    ...
    public PageResult<Map<String, Object>> listMovies(String keyword, Integer status, long pageNum, long pageSize) {
        ...
        queryArgs.add(paging.size());
        queryArgs.add(paging.offset());
        String sql = """
    ...
    @Override
    public void updateMovieStatus(Long id, Integer status) {
    ...
    public PageResult<Map<String, Object>> listUsers(String keyword, Integer status, long pageNum, long pageSize) {
        Paging p = paging(pageNum, pageSize);
        List<Object> args = new ArrayList<>();
        ...
        List<Object> qa = new ArrayList<>(args);
        qa.add(p.size());
        qa.add(p.offset());
        String sql = """
    ...
    public void resetUserPassword(Long id, String password) {
        if (!StringUtils.hasText(password) || password.length() < 6)
            throw new ResponseStatusException(BAD_REQUEST, "新密码不能少于 6 位");
        ensureChanged(jdbcTemplate.update("UPDATE user SET password=?,update_time=NOW() WHERE id=? AND deleted=0", passwordEncoder.encode(password), id), "用户不存在");
    }
    ...
    public PageResult<Map<String, Object>> listComments(String keyword, Long userId, Long movieId, long pageNum, long pageSize) {
        Paging p = paging(pageNum, pageSize);
        List<Object> args = new ArrayList<>();
        String where = " WHERE c.deleted=0";
        if (StringUtils.hasText(keyword)) {
        ...
        if (movieId != null) {
            where += " AND c.movie_id=?";
            args.add(movieId);
        }
        Long total = count("SELECT COUNT(*) FROM comment c" + where, args);
        List<Object> qa = new ArrayList<>(args);
        qa.add(p.size());
        qa.add(p.offset());
    ...
    public void deleteComment(Long id) {
        ensureChanged(jdbcTemplate.update("UPDATE comment SET deleted=1,update_time=NOW() WHERE id=? AND deleted=0", id), "评论不存在");
    }

    @Override
    public List<Map<String, Object>> listDictionaries(String type, String keyword) {
        String table = dictionaryTable(type);
        ...
        return jdbcTemplate.queryForList("SELECT id,name,description,status,create_time AS createTime FROM " + table + where + " ORDER BY update_time DESC,id DESC", args.toArray());
    }

    @Override
    public Long saveDictionary(String type, Long id, Map<String, Object> body) {
        String table = dictionaryTable(type);
        String name = text(body, "name");
        if (!StringUtils.hasText(name)) throw new ResponseStatusException(BAD_REQUEST, "名称不能为空");
        String description = text(body, "description");
        Integer status = intValue(body.get("status"), 1);
        if (id == null) {
            jdbcTemplate.update("INSERT INTO " + table + "(name,description,status,deleted,create_time,update_time) VALUES(?,?,?,0,NOW(),NOW())", name, description, status);
            return jdbcTemplate.queryForObject("SELECT LAST_INSERT_ID()", Long.class);
        }
        ensureChanged(jdbcTemplate.update("UPDATE " + table + " SET name=?,description=?,status=?,update_time=NOW() WHERE id=? AND deleted=0", name, description, status, id), "数据不存在");
        return id;
    }

    @Override
    public void updateDictionaryStatus(String type, Long id, Integer status) {
        String table = dictionaryTable(type);
        ensureChanged(jdbcTemplate.update("UPDATE " + table + " SET status=?,update_time=NOW() WHERE id=? AND deleted=0", normalizeStatus(status), id), "数据不存在");
    }

    @Override
    public void deleteDictionary(String type, Long id) {
        String table = dictionaryTable(type);
        String relation = table.equals("category") ? "movie_category" : "movie_tag";
        String column = table + "_id";
        Long used = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM " + relation + " WHERE " + column + "=?", Long.class, id);
        if (used != null && used > 0)
            throw new ResponseStatusException(BAD_REQUEST, "该项已被影片使用，请先禁用或解除绑定");
        ensureChanged(jdbcTemplate.update("UPDATE " + table + " SET deleted=1,update_time=NOW() WHERE id=? AND deleted=0", id), "数据不存在");
    }

    @Override
    public PageResult<Map<String, Object>> listPeople(String type, String keyword, long pageNum, long pageSize) {
        String table = personTable(type);
        Paging p = paging(pageNum, pageSize);
        List<Object> args = new ArrayList<>();
        String where = " WHERE deleted=0";
        if (StringUtils.hasText(keyword)) {
        ...
        qa.add(p.size());
        qa.add(p.offset());
        return page(total, p, jdbcTemplate.queryForList("SELECT id,name,original_name AS originalName,gender,birthday,nationality,profile_url AS profileUrl,biography FROM " + table + where + " ORDER BY update_time DESC,id DESC LIMIT ? OFFSET ?", qa.toArray()));
    }

    @Override
    public Long savePerson(String type, Long id, Map<String, Object> b) {
        String table = personTable(type);
        String name = text(b, "name");
        if (!StringUtils.hasText(name)) throw new ResponseStatusException(BAD_REQUEST, "姓名不能为空");
        Object[] values = {name, text(b, "originalName"), intValue(b.get("gender"), 0), b.get("birthday"), text(b, "nationality"), text(b, "profileUrl"), text(b, "biography")};
        if (id == null) {
            jdbcTemplate.update("INSERT INTO " + table + "(name,original_name,gender,birthday,nationality,profile_url,biography,deleted,create_time,update_time) VALUES(?,?,?,?,?,?,?,0,NOW(),NOW())", values);
            return jdbcTemplate.queryForObject("SELECT LAST_INSERT_ID()", Long.class);
        }
        List<Object> args = new ArrayList<>(List.of(values));
        args.add(id);
        ensureChanged(jdbcTemplate.update("UPDATE " + table + " SET name=?,original_name=?,gender=?,birthday=?,nationality=?,profile_url=?,biography=?,update_time=NOW() WHERE id=? AND deleted=0", args.toArray()), "人物不存在");
        return id;
    }

    @Override
    public void deletePerson(String type, Long id) {
        String table = personTable(type);
        ensureChanged(jdbcTemplate.update("UPDATE " + table + " SET deleted=1,update_time=NOW() WHERE id=? AND deleted=0", id), "人物不存在");
    }

    private void syncMovieRelations(Long id, AdminMovieRequest r) {
        sync("movie_category", "category_id", id, r.getCategoryIds());
        sync("movie_tag", "tag_id", id, r.getTagIds());
        sync("movie_actor", "actor_id", id, r.getActorIds());
        sync("movie_director", "director_id", id, r.getDirectorIds());
    }

    private void sync(String table, String column, Long movieId, List<Long> ids) {
        jdbcTemplate.update("DELETE FROM " + table + " WHERE movie_id=?", movieId);
        if (ids != null)
            ids.stream().distinct().forEach(id -> jdbcTemplate.update("INSERT INTO " + table + "(movie_id," + column + ") VALUES(?,?)", movieId, id));
    }

    private List<Long> relationIds(String table, String column, Long movieId) {
        return jdbcTemplate.query("SELECT " + column + " FROM " + table + " WHERE movie_id=?", (rs, n) -> rs.getLong(1), movieId);
    }

    private String dictionaryTable(String type) {
        return switch (type) {
            case "categories" -> "category";
            case "tags" -> "tag";
            default -> throw new ResponseStatusException(BAD_REQUEST, "不支持的数据类型");
        };
    }

    private String personTable(String type) {
        return switch (type) {
            case "actors" -> "actor";
            case "directors" -> "director";
            default -> throw new ResponseStatusException(BAD_REQUEST, "不支持的人物类型");
        };
    }

    private Paging paging(long number, long size) {
        return new Paging(Math.max(number, 1), Math.max(1, Math.min(size, 100)));
    }
    ...
    private <T> PageResult<T> page(Long total, Paging p, List<T> records) {
        return new PageResult<>(total, p.number(), p.size(), records);
    }

    private void ensureChanged(int count, String message) {
        if (count == 0) throw new ResponseStatusException(NOT_FOUND, message);
    }
    ...
    private Object value(Object value, Object fallback) {
        return value == null ? fallback : value;
    }

    private Date date(java.time.LocalDate value) {
    ...
    private record Paging(long number, long size) {
        long offset() {
