...
public class AdminManagementServiceImpl implements AdminManagementService {
    ...
    @Override public PageResult<Map<String,Object>> listAnnouncements(String keyword,long pageNum,long pageSize){Paging p=paging(pageNum,pageSize);List<Object> args=new ArrayList<>();String where=" WHERE deleted=0";if(StringUtils.hasText(keyword)){where+=" AND (title LIKE ? OR content LIKE ?)";String v="%"+keyword.trim()+"%";args.add(v);args.add(v);}Long total=count("SELECT COUNT(*) FROM announcement"+where,args);List<Object> qa=new ArrayList<>(args);qa.add(p.size());qa.add(p.offset());return page(total,p,jdbcTemplate.queryForList("SELECT id,title,content,status,create_time AS createTime,update_time AS updateTime FROM announcement"+where+" ORDER BY update_time DESC LIMIT ? OFFSET ?",qa.toArray()));}
    @Override public Long saveAnnouncement(Long id,Map<String,Object>b){String title=text(b,"title"),content=text(b,"content");if(!StringUtils.hasText(title)||!StringUtils.hasText(content))throw new ResponseStatusException(BAD_REQUEST,"公告标题和内容不能为空");Integer status=intValue(b.get("status"),1);if(id==null){jdbcTemplate.update("INSERT INTO announcement(title,content,status,deleted,create_time,update_time) VALUES(?,?,?,0,NOW(),NOW())",title,content,status);return jdbcTemplate.queryForObject("SELECT LAST_INSERT_ID()",Long.class);}ensureChanged(jdbcTemplate.update("UPDATE announcement SET title=?,content=?,status=?,update_time=NOW() WHERE id=? AND deleted=0",title,content,status,id),"公告不存在");return id;}
    @Override public void deleteAnnouncement(Long id){ensureChanged(jdbcTemplate.update("UPDATE announcement SET deleted=1,update_time=NOW() WHERE id=? AND deleted=0",id),"公告不存在");}
