...
@Service @RequiredArgsConstructor public class MovieCacheService{
 ...
 public void evictMovieCaches(){if(!enabled)return;try{StringRedisTemplate r=provider.getIfAvailable();Set<String>keys=new HashSet<>();for(String p:List.of("movie:home*","movie:hot*","movie:top-rated*","movie:latest*","movie:ranking:*","movie:similar:*","movie:detail:*")){Set<String>found=r.keys(p);if(found!=null)keys.addAll(found);}if(!keys.isEmpty())r.delete(keys);}catch(Exception ignored){}}
}
