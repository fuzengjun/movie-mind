  ...
  <RouterLink :to="`/movies/${movie.id}`" class="group block no-underline">
    ...
    <div class="poster-card-premium relative overflow-hidden">
      ...
      <div class="aspect-[2/3] overflow-hidden bg-[var(--bg-secondary)]">
        ...
        <!-- 缺省海报占位 -->
        <div v-else
             class="flex h-full w-full items-center justify-center bg-[var(--border-soft)] text-[var(--text-muted)]">
      ...
      <div
          class="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"></div>
    </div>
    ...
    <div class="mt-2.5 px-0.5 text-left">
      ...
      <div class="mt-1 flex items-center justify-between text-xs text-[var(--text-secondary)] font-medium">
        ...
        <div class="flex items-center gap-2">
          <!-- 评分小胶囊 -->
          <span
              class="rounded bg-[var(--surface-secondary)] border border-[var(--border-soft)] px-1.5 py-0.5 text-[10px] font-bold text-[var(--text-primary)]">
