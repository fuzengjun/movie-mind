  ...
  <AdminPage title="基础资料" kicker="Metadata" hint="统一维护分类、标签、演员和导演。">
    ...
    <el-dialog v-model="dialog" :title="`${form.id ? '编辑' : '新增'}${current.label}`" width="min(620px,92vw)">
        ...
        <template v-if="isPerson">
          <div class="admin-form-grid">
            ...
            <el-form-item label="头像地址">
              <el-input v-model="form.profileUrl"/>
            </el-form-item>
          </div>
      ...
      <template #footer>
        ...
        <button class="pill-button is-active" @click="save">保存</button>
      </template>
    </el-dialog>
  </AdminPage>
</template>
...
const tabs = [{name: 'categories', label: '分类'}, {name: 'tags', label: '标签'}, {
  name: 'actors',
  label: '演员'
}, {name: 'directors', label: '导演'}]
const active = ref('categories'), keyword = ref(''), records = ref([]), total = ref(0), pageNum = ref(1),
    loading = ref(false), dialog = ref(false), form = reactive({})
...
const blank = () => isPerson.value ? {
  ...
  profileUrl: '',
  biography: ''
} : {id: null, name: '', description: '', status: 1}

async function load() {
  loading.value = true;
  try {
    if (isPerson.value) {
      const r = await pagePeople(active.value, {keyword: keyword.value, pageNum: pageNum.value, pageSize: 10});
      records.value = r.data.records;
      total.value = r.data.total
    } else records.value = (await listDictionaries(active.value, {keyword: keyword.value})).data
  } finally {
    loading.value = false
  }
}
...
async function save() {
  if (isPerson.value) await savePerson(active.value, form.id, form); else await saveDictionary(active.value, form.id, form);
...
async function remove(row) {
  ...
  if (isPerson.value) await deletePerson(active.value, row.id); else await deleteDictionary(active.value, row.id);
